...
# monthly.close.price <- as.ts(monthly.close.price)
...
monthly.kdj.k <- ts(monthly.original.data[,7], start = c(1990, 1), frequency = 12)
monthly.kdj.d <- ts(monthly.original.data[,8], start = c(1990, 1), frequency = 12)
```

#### 1.1 Forecast result of monthly kdj k/d 

```{r main 1.2, fig.height=6, fig.width=10, echo = FALSE, cache=TRUE}
...
#### 1.3 Data of Recent months 


```{r appendix 1.3.2, fig.height=2, fig.width=7, echo = FALSE, cache=FALSE, eval=TRUE}
recent.month.count <- 5

recent.monthly.return <- round(tail(monthly.compound.return, recent.month.count),digits = 2)

sigma <- round(sqrt(fit.arima.monthly.return$sigma2),digits = 2)

flag.return.larger.than.sigma <- (abs(recent.monthly.return) > sigma)

d <- data.frame(recent.monthly.return = paste0(recent.monthly.return,"%"),
                sigma = paste0(sigma,"%"),
                absolute.return.larger.than.sigma = flag.return.larger.than.sigma)
...
```{r appendix 2.3.2, fig.height=2, fig.width=7, echo = FALSE, cache=FALSE, eval=TRUE}
recent.week.count <- 5

recent.weekly.return <- round(tail(weekly.compound.return, recent.week.count),digits = 2)

sigma <- round(tail(myfilter.aparch.std.52.ver1@filter$sigma,recent.week.count),digits = 2)

flag.return.larger.than.sigma <- (abs(recent.weekly.return) > sigma)
