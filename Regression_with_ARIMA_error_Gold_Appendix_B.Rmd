...
It can be concluded that pure Arima model with max.order set to 5 and d set to 0 is well for predicting weekly close.price.
...
```{r appendix 2.1-2, echo = FALSE, cache=TRUE, eval=TRUE}
...
reg.lagged.df <- data.frame(cbind(weekly.close.price,
                                  ts(c(NA, weekly.close.price[-length(weekly.close.price)])),
                                  ts(c(NA, NA, head(weekly.close.price, length(weekly.close.price) - 2)))))
...
```{r appendix 3.5, fig.height=5, fig.width=7, echo = TRUE, cache=FALSE, eval=FALSE}
...
```{r appendix 3.6, fig.height=5, fig.width=7, echo = TRUE, cache=TRUE, eval=FALSE}
setwd("d://MyR//stock//")
source("CompareObjectNaiveAccuracy.R")
...
```{r appendix 3.6(2), fig.height=5, fig.width=7, echo = TRUE, cache=FALSE, eval=FALSE}
